...
def buscar_produtos_por_nome(nome_produto):
    ...
    if result:
        for row in result:
            with st.expander(f"ID do Produto: {row[0]} - {row[1]}"):
                id_ = f"{row[0]}"
                st.markdown(f"""
                            ```copiar
                            {id_}
                ...
                st.write(f"**Quantidade em Estoque:** {row[13]}")
                
                
                # Exibe a imagem com tamanho padrão e verifica possíveis erros
                try:
                    st.image(row[11], width=180, caption=f"Imagem do Produto {row[1]}")
                except:
                ...
                st.write(f"**Status Anúncio** {row[23]}")
                st.write("---")
    else:
